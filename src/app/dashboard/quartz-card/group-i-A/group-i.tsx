...
import { groupia } from "./data";

function Group_IA() {
  const [selectedId, setSelectedId] = useState<number | null>(null);

  const selectedItem = groupia.find((d) => d.id === selectedId);

  return (
    <div className="relative border border-b">
      <div className={`${selectedItem ? "blur-sm" : ""}`}>
        <p className="px-4 font-bold">Group-I-A</p>
          ...
          {groupia.map((d) => (
            ...
            >
              <CardContent className="p-0 overflow-hidden w-full">
                <div className="relative w-full aspect-[1/1.918] overflow-hidden">
                  <Image
                    src={d.img}
                    alt={d.label}
                    fill
