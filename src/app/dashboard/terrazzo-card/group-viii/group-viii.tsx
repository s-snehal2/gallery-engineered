...
import { groupviii } from "./data";

function Group_VIII() {
  const [selectedId, setSelectedId] = useState<number | null>(null);

  const selectedItem = groupviii.find((d) => d.id === selectedId);

  return (
    <div className="relative border border-b">
      {/* GRID VIEW */}
      <div className={`${selectedItem ? "blur-sm" : ""}`}>
        <p className="px-4 font-bold">Group-VIII</p>
      ...
      {selectedItem && (
        <div
          className="fixed inset-0 bg-primary/70 backdrop-blur-sm flex items-center justify-center z-50"
